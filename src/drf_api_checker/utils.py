...
def _write(dest, content):
    if isinstance(dest, str):
        ...
        raise ValueError(
            f"'dest' must be a filepath or file-like object. It is {type(dest)}"
        )
...
def _read(source):
    if isinstance(source, str):
        ...
        return source.read()
    raise ValueError(
        f"'source' must be a filepath or file-like object. It is {type(source)}"
    )
...
def dump_fixtures(fixtures, destination):
    ...
    for k, instances in fixtures.items():
        ...
        if isinstance(instances, (list, tuple)):
            ...
            ret = j.serialize(collector.data, use_natural_foreign_keys=False)
            data[k] = {"master": json.loads(ret)[0], "deps": json.loads(ret)[1:]}
